...
class Florence2LoRAModelTrainer:
    ...
    def merge_and_save_model(self, output_path: str):
        ...
        try:
            ...
            merged_model.save_pretrained(output_path) # save model not
            ...
            merge_record = {
                ...
                "note": "Merged model ready for direct use - no LoRA adapter needed"
            }
            ...
            print(f"ℹ️  This merged model can be used directly without LoRA adapters")
...
def main():
    ...
    try:
        ...
        if hasattr(main, '_merge_after_training') and main._merge_after_training:
            print("\n5. Merging LoRA with base model...")
            merge_path = "weights/icon_caption_florence_merged"
            if trainer.merge_and_save_model(merge_path):
...
Examples:
  ...
  python finetune_omniparser_lora.py --data training_data/my_data.json --merge
        """
    )
    ...
    parser.add_argument(
        ...
        help="Path to training data JSON file (default: training_data/florence_format/florence_data.json)"
    )
    ...
    parser.add_argument(
        ...
        help="Path to base Florence2 model (default: weights/icon_caption_florence)"
    )
    ...
    parser.add_argument(
        ...
        help="Output path for merged model (default: weights/icon_caption_florence_merged)"
    )
    ...
    parser.add_argument(
        ...
        help="Path to existing LoRA adapter for merge-only mode (default: weights/icon_caption_florence_lora_finetuned)"
    )
...
def merge_existing_lora(lora_path: str, base_model_path: str, output_path: str):
    ...
    try:
        ...
        merge_info = {
            ...
            "merge_timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
            "note": "Merged from existing LoRA adapter - ready for direct use"
        }
        ...
        print(f"ℹ️  Usage: model = AutoModelForCausalLM.from_pretrained('{output_path}')")
...
if __name__ == "__main__":
    ...
    # 传递merge选项给main函数
    main._merge_after_training = args.merge
    main._merge_path = args.merge_path
        ...
        try:
            ...
            if args.merge:
                ...
                if trainer.merge_and_save_model(args.merge_path):
                    print(f"✓ Merged model saved to {args.merge_path}")
                    print(f"ℹ️  You can now use the merged model directly:")
                    print(f"ℹ️  model = AutoModelForCausalLM.from_pretrained('{args.merge_path}')")
                else:
                    print("✗ Model merge failed")
